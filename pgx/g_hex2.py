...
from pgx._src.games.g_hex2 import Game, GameState
...
class State(core.State):
    ...
    observation: Array = jnp.ones((3, 4+2*11), dtype=jnp.int32)
    rewards: Array = jnp.float32([0.0, 0.0])
    terminated: Array = jnp.bool_(False)
    truncated: Array = jnp.bool_(False)
    legal_action_mask: Array = jnp.tile(jnp.ones(11, dtype=jnp.bool_), 23)
...
def black2(tile_val, tri):
    assert tile_val >= 1
    assert tile_val <= 10 or tile_val == 20
    assert tri >= 0
    assert tri <= 22

    return tri * 11 + (10 if tile_val == 20 else tile_val - 1)


class GHex2(core.Env):
